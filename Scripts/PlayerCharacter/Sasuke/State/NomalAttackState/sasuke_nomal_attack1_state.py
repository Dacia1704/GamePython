...
class SasukeNomalAttack1State(NomalAttackState):

  def enter(self):
    super().enter()
    self.attack_collider_animations = GameConstants.SASUKE_ATTACK1_COLLIDER_ANIMATIONS
  ...
  def draw(self, surface):
    super().draw(surface)
    self.draw_attack_animation(surface,self.state_machine.character.nomal_attack1_spritesheet,GameConstants.SASUKE_ATTACK1_COLLIDER_DICTIONARY)
