...
class SasukeSkill1State(SkillState):
  ...
  def draw(self, surface):
    super().draw(surface)
    self.draw_skill_animation(surface,self.state_machine.character.skill1_spritesheet,GameConstants.SASUKE_SKILL1_COLLIDER_DICTIONARY)
  ...
  def draw_skill_animation(self, surface,sprite_sheet_data,animation_collider_dictionary):
    ...
    if self.current_sprite_index == len(skill_sprite_sheet[0])-1 and not self.is_fire_energy_ball:
      ...
      posX = (self.state_machine.character.rect.centerx + 70) if not self.state_machine.character.flip else (self.state_machine.character.rect.centerx - 70 - GameConstants.SASUKE_ENERGY_BALL_SIZE[0])
      
      energyball = EnergyBall([posX,self.state_machine.character.rect.centery-42,GameConstants.SASUKE_ENERGY_BALL_SIZE[0],GameConstants.SASUKE_ENERGY_BALL_SIZE[1]],[-GameConstants.SASUKE_ENERGY_BALL_SPEED if self.state_machine.character.flip else GameConstants.SASUKE_ENERGY_BALL_SPEED,0],self.state_machine.character,self.state_machine.character.target,self.state_machine.screen_surface,self.state_machine.character.energy_ball_sprite_sheet)
