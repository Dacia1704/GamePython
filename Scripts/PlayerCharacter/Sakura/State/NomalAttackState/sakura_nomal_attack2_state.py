...
class SakuraNomalAttack2State(NomalAttackState):

  def enter(self):
    super().enter()
    AudioManager.get_instance().play_sfx(self.state_machine.character.nomal_attack_2_sfx_name)
    self.attack_collider_animations = GameConstants.SAKURA_ATTACK2_COLLIDER_ANIMATIONS
  ...
  def draw(self, surface):
    super().draw(surface)
    self.draw_attack_animation(surface,self.state_machine.character.nomal_attack2_spritesheet,GameConstants.SAKURA_ATTACK2_COLLIDER_DICTIONARY)